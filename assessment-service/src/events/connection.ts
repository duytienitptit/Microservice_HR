...
export async function connectRabbitMQ(url: string = config.rabbitmq.url): Promise<Channel> {
  ...
  try {
    ...
    // Assert exchanges
    await activeChannel.assertExchange('interview.events', 'direct', { durable: true });
    ...
    // Assert DLQs
    await activeChannel.assertQueue('dlq.interview.completed', { durable: true });
    await activeChannel.assertQueue('dlq.send.email', { durable: true });

    // Bind DLQs to DLX
    await activeChannel.bindQueue('dlq.interview.completed', 'dlx.events', 'interview.completed');
    await activeChannel.bindQueue('dlq.send.email', 'dlx.events', 'send.email');

    // Assert and bind main queues
    await activeChannel.assertQueue('interview.completed', {
      durable: true,
      arguments: {
    ...
    await activeChannel.bindQueue('interview.completed', 'interview.events', 'interview.completed');
